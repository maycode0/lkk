...
def evaluation(res,query_budget):
    ...
    total_query = 0
    for i in res:
        total_query += result_total_queries(i)
        if i[0] == 2:
            atk+=1
            #count+=1
        if i[0] == 1:
...
def run_attack():
    ...
    parser.add_argument("--target_model",
                        ...
                        help="Target model for text classification: wordCNN, wordLSTM, bert, roberta.")   # TODO
    
    parser.add_argument("--model_path",
                        type=str,
                        default='cnn/cnn_mr',
                        help="Path/name of the target model checkpoint.")   # TODO
    ...
    parser.add_argument("--max_seq_length",
                        type=int,
                        default=256,
                        help="Maximum input sequence length for transformer target models.")
    ...
    if args.target_model == "wordCNN":
        ...
        if 'ag' in args.dataset_path:
            ...
            default_model_path = os.path.join(default_model_path,args.model_path)
        model = infer_cls(default_model_path, nclasses=args.nclasses, max_seq_length=args.max_seq_length)
