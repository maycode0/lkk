...
def _require_transformers():
    if AutoModelForSequenceClassification is None or AutoTokenizer is None:
        raise ImportError(
            "RoBERTa target models require the `transformers` package. "
            "Install it before running with `--target_model roberta`."
        )


def _as_text_batch(text_data):
    texts = []
...
def _hard_label_from_probs(probs):
    probs = probs.detach().cpu()
    hard_label = torch.zeros(probs.shape)
    maxvalue = torch.argmax(probs, dim=1)
    for i in range(len(hard_label)):
        hard_label[i][maxvalue[i]] = 1
    return hard_label.numpy()
...
class NLI_infer_RoBERTa(nn.Module):
    def __init__(self,
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_dir, use_fast=True)
        self.model = AutoModelForSequenceClassification.from_pretrained(
            pretrained_dir,
            num_labels=nclasses
        )
        self.model = self.model.to(device)
        self.model_forward_args = set(inspect.signature(self.model.forward).parameters)

    def _predict_logits(self, text_data, batch_size=32):
        self.model.eval()
        texts = _as_text_batch(text_data)
        logits_all = []
        ...
        for i in range(0, len(texts), batch_size):
            ...
            encoded = {
                ...
                if key in self.model_forward_args
            }
            with torch.no_grad():
        ...
        if not logits_all:
            ...
            return torch.empty((0, num_labels), device=device)

        return torch.cat(logits_all, dim=0)

    def text_pred(self, text_data, batch_size=32):
        logits = self._predict_logits(text_data, batch_size=batch_size)
        return nn.functional.softmax(logits, dim=-1)

    def lime_text_pred(self, text_data, batch_size=32):
        probs = self.text_pred(text_data, batch_size=batch_size)
        return _hard_label_from_probs(probs)
